...
typedef void (*FlatpakInternalProgressCallback) (FlatpakProgress *self,
                                                 gpointer         user_data);

FlatpakProgress *flatpak_progress_new (FlatpakInternalProgressCallback callback,
                                       gpointer                        user_data,
                                       GDestroyNotify                  user_data_free_func);

void flatpak_progress_init_initializing (FlatpakProgress *self);
...
typedef OstreeAsyncProgress FlatpakOstreeAsyncProgress;

FlatpakMainContext *flatpak_progress_push_main_context (FlatpakProgress *maybe_progress);
FlatpakOstreeAsyncProgress *flatpak_progress_issue_ostree_progress (FlatpakProgress *self);
void flatpak_progress_revoke_ostree_progress (FlatpakProgress     *self,
                                              OstreeAsyncProgress *ostree_progress);

static inline void
ostree_progress_cleanup (OstreeAsyncProgress *ostree_progress)
{
  FlatpakProgress *progress = g_object_get_data (G_OBJECT (ostree_progress), "flatpak-progress");
  flatpak_progress_revoke_ostree_progress (progress, ostree_progress);
}

G_DEFINE_AUTOPTR_CLEANUP_FUNC (FlatpakOstreeAsyncProgress, ostree_progress_cleanup);
