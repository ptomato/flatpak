...
struct _FlatpakProgress
{
  ...
  /* Callback */
  FlatpakInternalProgressCallback callback;
  gpointer                        user_data;
  GDestroyNotify                  user_data_free_func;

  char *status;  /* derived from the ostree_status */
  ...
  /* Flags */
  gboolean  extra_data_inited      : 1;
  gboolean  oci_pull_inited        : 1;
...
flatpak_progress_finalize (GObject *object)
{
  ...
  if (self->user_data_free_func)
    g_clear_pointer (&self->user_data, self->user_data_free_func);
...
FlatpakProgress *
flatpak_progress_new (FlatpakInternalProgressCallback callback,
                      gpointer                        user_data,
                      GDestroyNotify                  user_data_free_func)
{
  ...
  retval->user_data_free_func = user_data_free_func;

  return retval;
}

void
flatpak_progress_init_initializing (FlatpakProgress *self)
{
  self->ostree_status = g_strdup ("Initializing");
...
update_status_progress_and_estimating (FlatpakProgress *self)
{
  ...
  if (self->ostree_status && *self->ostree_status && self->total_extra_data_bytes == 0)
    {
  ...
  if (self->total_delta_parts == 0 &&
      (self->outstanding_metadata_fetches > 0 || self->last_was_metadata)  &&
                                  ...
                                  guint64 total_download_size)
{
  ...
  self->extra_data_inited = TRUE;
  self->progress = 0;
  update_status_progress_and_estimating (self);
}

gboolean
flatpak_progress_get_extra_data_initialized (FlatpakProgress *self)
{
  return self->extra_data_inited;
}
                 ...
                 FlatpakProgress     *progress)
{
  ...
  progress->callback (progress, progress->user_data);
}

FlatpakOstreeAsyncProgress *
